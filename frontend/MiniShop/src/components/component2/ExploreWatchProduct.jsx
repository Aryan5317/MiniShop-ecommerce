...
function ExploreWatchProduct() {
    const navigate = useNavigate()
    const [productData, setProductData] = useState([])

    useEffect(() => {
        const products = async () => {
            const productDetails = await watchDetailsService();
            console.log("Watch details: ", productDetails.data.productData)

            // Sort watches by discount (highest first)
        ...
        <div className="overflow-x-auto">
            <div className="flex flex-row w-max pb-15">
                {productData.map((data) => (
                    <div
                        key={data._id}
                        onClick={() => navigateWatchPage(data._id)}
                    ...
                    >
                        ...
                        <div className="mt-1 flex">
                            <p className="text-sky-600 text-left line-clamp-4 text-md font-semibold">
                                {data.watchName} ({data.color}) | {data.strapMaterial} | {data.dialSize}mm
                            </p>
                        </div>
...
export default ExploreWatchProduct
