...
function ExploreShoesProducts() {
    const navigate = useNavigate()
    const [productData, setProductData] = useState([])

    useEffect(() => {
        const products = async () => {
            const productDetails = await shoesDetailsService();
    ...
    const navigateShoeDetailsPage = (id) => {
        ...
        <div className="overflow-x-auto">
            <div className="flex flex-row w-max pb-15">
                {productData.map((data) => (
                    <div
                        key={data._id}
                        onClick={() => navigateShoeDetailsPage(data._id)}
                    ...
                    >
                        ...
                        <div className="w-full flex justify-center overflow-hidden h-[10vh] sm:h-[25vh] lg:h-[32vh]">
                            <img src={data.shoeImages?.[0]} alt={data.shoeName} className="h-full w-full object-contain" />
                        </div>

                        {/* Shoe Name + Details */}
                        <div className="mt-1 flex">
                            <p className="text-sky-600 text-left line-clamp-4 text-md font-semibold">
                                {data.shoeName} ({data.color}, {data.shoeType}) | {data.material} | {data.description.highlights}
                            </p>
                        </div>
...
export default ExploreShoesProducts
