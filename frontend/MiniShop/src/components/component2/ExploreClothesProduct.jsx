...
function ExploreClothesProduct() {
    const navigate = useNavigate()
    const [productData, setProductData] = useState([])

    useEffect(() => {
        const products = async () => {
            const productDetails = await clothDetailsService();
            console.log("Clothes details: ", productDetails.data.productData)

            // Sort clothes by discount (highest first)
    ...
    const navigatClothesPage = (id) => {
        console.log("Navigating to clothes: ", id)
        navigate(`/clothes/${id}`)
        window.scrollTo(0, 0)
    }
        ...
        <div className="overflow-x-auto">
            <div className="flex flex-row w-max pb-15">
                {productData.map((data) => (
                    <div
                        key={data._id}
                        onClick={() => navigatClothesPage(data._id)}
                    ...
                    >
                        ...
                        {/* Clothing Name & Details */}
                        <div className="mt-1 flex">
                            <p className="text-sky-600 text-left line-clamp-4 text-md font-semibold">
                                {data.clothingName} ({data.color}) | {data.productCategory} | {data.fit} | {data.sleeveType}
                            </p>
                        </div>
...
export default ExploreClothesProduct
