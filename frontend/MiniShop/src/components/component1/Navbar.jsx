...
function Navbar() {
    const { isLoggedIn, storeInputValue, setStoreInputValue } = useContext(propContext)
    ...
    useEffect(() => {
      console.log("Store value: ", storeInputValue)
    }, [storeInputValue])
    ...
    return (
        <div className="w-full mb-1 pt-2 pb-2 flex flex-col gap-4 shadow-sm md:flex-row md:items-center md:justify-between bg-blue-200/50 transition-all duration-200 hover:shadow-md">
            ...
            <div className="flex hidden md:flex gap-3 font-semibold mr-7">
                ...
                {isLoggedIn && (<div className="flex justify-center items-center justify-center">
                    <h3 className="text-sky-600 pl-3"><FiMapPin /></h3>
                    <Link to="/login" className="p-1 px-3 rounded-md text-gray-600 text-xl ">123456</Link>
                </div>)}
