...
function SelectedBagPage() {
    ...
    const paymentButton = (id, category) => {
        ...
        if (locationFlag) {
            ...
            setTimeout(() => {
                navigate(`/product/${product}/orderSummary?quantity=${itemQuantity}&category=${"Bags"}`)
            }, 2000)
        } else {
        ...
        <>
                ...
                <div className="border hidden md:flex flex-row m-3 mt-1 bg-white">
                    ...
                    <div className="mx-3 pt-3 w-[55%] flex flex-col">
                        <p className="flex font-medium text-xl">
                            {bagData.bagName} ({bagData.color}) | {bagData.bagType} | {bagData.capacity} | {bagData.material} | {bagData.description?.highlights}
                        </p>
                        ...
                        <div className="flex mt-3 gap-1">
                            <h3 className="font-bold text-md">{bagData.totalSell}+ bought</h3>
                        ...
                        <div className="mt-5 m-3 mb-0 flex flex-row gap-3">
                            <h3 className="font-medium text-2xl text-red-400">-{bagData.discount}%</h3>
                            <div className="flex">
                                <h3>₹</h3>
                                <h3 className="text-3xl font-semibold">{bagData.currentPrice}</h3>
                            </div>
                        </div>
                        <div className="m-3 mt-1 flex gap-1">
                            <h3>M.R.P:</h3>
                            <h3 className="line-through">₹{bagData.oldPrice}</h3>
                        ...
                        <div className="flex flex-col gap-3 mt-3 mx-3">
                            ...
                            <div className="flex border px-3 py-2 w-[40%] gap-1 rounded-3xl bg-yellow-400 items-center justify-center hover:bg-yellow-500 transition duration-300">
                                {!cartValue
                                    ? <button className="text-lg" onClick={() => addToCart(bagData._id, bagData.productCategory)}>Add to cart</button>
                            ...
                            <div className="flex border px-3 py-2 mb-3 w-[40%] gap-1 rounded-3xl bg-amber-500 items-center justify-center hover:bg-orange-500 transition duration-300">
                                <button className="text-lg" onClick={() => paymentButton(bagData._id, bagData.productCategory)}>Buy now</button>
                            </div>
                        </div>
                        <div className="mt-3 flex flex-col">
                            <h3 className="text-xl font-bold">About this item</h3>
                            <ul className="list-disc pl-4">
                                <li>{bagData.description?.highlights}</li>
                                <li>Compartments: {bagData.description?.compartments}</li>
                                <li>Material: {bagData.description?.material}</li>
                                <li>Dimensions: {bagData.description?.dimensions}</li>
                                <li>{bagData.description?.extras}</li>
                            </ul>
                        </div>
                    </div>
                </div>
            ...
            <div className="border flex flex-col md:hidden">
                ...
                <div className="flex items-center justify-center m-3 mt-1 py-3">
                    <img src={bagData.bagImages?.[0]} alt="Error 404" className="w-[40%]" />
                </div>
                ...
                <div className="flex flex-row m-3 mt-1 gap-3 items-center">
                    <p className="font-semibold text-xl line-clamp-2 break-words px-3">
                        {bagData.bagName} ({bagData.color}) | {bagData.bagType} | {bagData.capacity}
                    </p>
                </div>
                <div className="flex flex-row m-3 mt-1 gap-3 items-center">
                    <h3 className="text-2xl text-red-600">-{bagData.discount}%</h3>
                    <div className="flex items-center">
                        <h3>₹</h3>
                        <h4 className="text-3xl font-semibold">{bagData.currentPrice}</h4>
                    </div>
                </div>
