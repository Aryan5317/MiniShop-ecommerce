...
function SelectedClothesPage() {
    ...
    const paymentButton = () => {
        ...
        if (locationFlag) {
            ...
            setTimeout(() => {
                navigate(`/product/${product}/orderSummary?quantity=${itemQuantity}&category=Clothes`)
            }, 2000)
        } else {
        ...
        <>
            ...
            <div className="hidden md:flex flex-col">
                ...
                <div className="border hidden md:flex flex-row m-3 mt-1 bg-white">
                    ...
                    <div className="h-[50vh] lg:h-[100vh] w-[35%] lg:w-[40%] flex items-center justify-center">
                        <img
                            src={clothData.clothingImages?.[0]}
                            alt="Error 404"
                    ...
                    <div className="mx-3 pt-3 w-[55%] flex flex-col">
                        ...
                        <p className="flex font-medium text-xl">
                            {clothData.clothingName} ({clothData.color}) | {clothData.productCategory} | {clothData.fit} | {clothData.sleeveType} | {clothData.fabric} | {clothData.description?.highlights}
                        </p>
                        ...
                        <div className="flex mt-3 gap-1">
                            <h3 className="font-bold text-md">{clothData.totalSell}+ bought</h3>
                        ...
                        <div className="mt-5 m-3 mb-0 flex flex-row gap-3">
                            <h3 className="font-medium text-2xl text-red-400">-{clothData.discount}%</h3>
                            <div className="flex">
                                <h3>₹</h3>
                                <h3 className="text-3xl font-semibold">{clothData.currentPrice}</h3>
                            </div>
                        </div>
                        <div className="m-3 mt-1 flex gap-1">
                            <h3>M.R.P:</h3>
                            <h3 className="line-through">₹{clothData.oldPrice}</h3>
                        ...
                        <div className="flex flex-col gap-3 mt-3 mx-3">
                            ...
                            <div className="flex border px-3 py-2 w-[40%] gap-1 rounded-3xl bg-yellow-400 items-center justify-center hover:bg-yellow-500 transition duration-300">
                                {!cartValue
                                    ? <button className="text-lg" onClick={() => addToCart(clothData._id, clothData.productCategory)}>Add to cart</button>
                        ...
                        {/* About this item — clothes schema fields only */}
                        <div className="mt-3 flex flex-col">
                            <h3 className="text-xl font-bold">About this item</h3>
                            <ul className="list-disc pl-4">
                                <li>{clothData.description?.highlights}</li>
                                <li>Material: {clothData.description?.material}</li>
                                <li>Fit: {clothData.description?.fit}</li>
                                <li>Care: {clothData.description?.care}</li>
                                <li>{clothData.description?.extras}</li>
                                {clothData.neckType && <li>Neck Type: {clothData.neckType}</li>}
                                {clothData.collarType && <li>Collar Type: {clothData.collarType}</li>}
                                {clothData.occasion && <li>Occasion: {clothData.occasion}</li>}
                            </ul>
                        </div>
                    </div>
                </div>
            ...
            <div className="border flex flex-col md:hidden">
                ...
                <div className="flex items-center justify-center m-3 mt-1 py-3">
                    <img src={clothData.clothingImages?.[0]} alt="Error 404" className="w-[40%]" />
                </div>
                ...
                <div className="flex flex-row m-3 mt-1 gap-3 items-center">
                    <p className="font-semibold text-xl line-clamp-2 break-words px-3">
                        {clothData.clothingName} ({clothData.color}) | {clothData.productCategory} | {clothData.fit}
                    </p>
                </div>

                {/* Price */}
                <div className="flex flex-row m-3 mt-1 gap-3 items-center">
                    <h3 className="text-2xl text-red-600">-{clothData.discount}%</h3>
                    <div className="flex items-center">
                        <h3>₹</h3>
                        <h4 className="text-3xl font-semibold">{clothData.currentPrice}</h4>
                    </div>
                </div>
...
export default SelectedClothesPage