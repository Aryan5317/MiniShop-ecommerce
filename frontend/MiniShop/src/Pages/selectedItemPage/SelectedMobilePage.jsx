...
function SelectedMobilePage() {
  ...
  const paymentButton = (id, category) => {
    ...
    if (locationFlag) {
      ...
      setTimeout(() => {
        navigate(`/product/${product}/orderSummary?quantity=${itemQuantity}&category=${"Phones"}`)
      }, 2000)
    } else {
    ...
    <>
          ...
          <div className="mx-3 pt-3 w-[55%] flex flex-col ">
            ...
            <div className="flex flex-col gap-3 mt-3 mx-3 ">
              ...
              <div className="flex border px-3 py-2 w-[40%] gap-1 rounded-3xl bg-yellow-400 items-center justify-center hover:bg-yellow-500 transition duration-300">
                {!cartValue && <button className="text-lg" onClick={() => addToCart(phoneData._id, phoneData.productCategory)}>Add to cart</button>}
              ...
              <div className="flex border px-3 py-2 mb-3 w-[40%] gap-1 rounded-3xl bg-amber-500 items-center justify-center hover:bg-orange-500 transition duration-300">
                <button className="text-lg" onClick={() => paymentButton(phoneData._id, phoneData.productCategory)}>Buy now</button>
              </div>
            </div>
      ...
      <div className="border flex flex-col md:hidden">
        ...
        <div className="flex items-center justify-center m-3 mt-1 py-3">
          <img src={phoneData.phoneImages?.[0]} alt="Error 404" className="w-[40%]" />
        </div>
        <div className="py-[2px] bg-gray-200"></div>
        <div className="flex flex-row m-3 mt-1 gap-3 items-center">
          <p className="font-semibold text-xl line-clamp-2 break-words px-3">{phoneData.phoneName} ({phoneData.color}, {phoneData.phoneRam}GB + {phoneData.phoneStorage}GB)</p>
        </div>
        <div className="flex flex-row m-3 mt-1 gap-3 items-center">
          <h3 className="text-2xl text-red-600">-{phoneData.discount}%</h3>
          <div className="flex items-center">
            <h3>₹</h3>
            <h4 className="text-3xl font-semibold">{phoneData.currentPrice}</h4>
          </div>
        </div>
